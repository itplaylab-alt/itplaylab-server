// index.js — ITPlayLab 통합본 Hotfix (승인 루프 + 한글 알림 + 체크리스트 + GAS 로깅 + 대시보드 + 리비전 3회 제한)
// ✅ Responses API 우선 사용 + 자동 Fallback(chat.completions) + 스키마 검증(AJV, 미설치 시 자동 스킵)
...
const OPENAI_MODEL_RESP = process.env.OPENAI_MODEL_RESP || "gpt-4.1-mini"; // Responses API
const OPENAI_MODEL_FALLBACK = process.env.OPENAI_MODEL_FALLBACK || "gpt-4o-mini"; // chat.completions
...
function buildRevisionPrompts(stepName, trace, reason = "", checks = []) {
  ...
  if (stepName === "brief")
    return {
      system: "너는 콘텐츠 프로듀서다. 이전 브리프를 피드백에 맞게 최소 수정. JSON만 출력.",
  ...
  if (stepName === "script")
    return {
      system: "너는 숏폼 스크립트라이터다. 총60초 원칙 유지하며 피드백 반영. JSON만 출력.",
  ...
  if (stepName === "assets")
    return {
      system: "너는 유튜브 운영자다. 메타를 피드백에 맞게 개선. JSON만 출력.",
...
async function redoCurrentStepWithRevision(trace, { reason = "", checks = [], by = "inline" } = {}) {
  ...
  if (trace.revisionCount >= MAX_REVISIONS) {
    ...
    await logToSheet({
      ...
      input_text: trace.title,
      output_text: { last_output: trace.lastOutput?.[stepName] || {}, reason, checks },
      project: PROJECT,
      category: "revision",
      note: `trace=${trace.id}`,
      trace_id: trace.id,
      ...
      revision_count: trace.revisionCount,
    });
    return;
  }
  ...
  trace.history.push({
    step: `${stepName}_revision_${trace.revisionCount}`,
    ok: true,
    latency_ms,
    provider: r.provider,
    startedAt,
  ...
  if (shouldNotify("success")) {
    const msg = [
      fmtTitle(trace.title),
      fmtTrace(trace.id),
      `수정 단계: <b>${labelStep(stepName)}</b>`,
      `수정 회차: <b>${trace.revisionCount}</b> / ${MAX_REVISIONS}`,
      `지연시간: <code>${latency_ms}ms</code>`,
      `엔진: <code>${r.provider}</code>`,
    ].join("\n");
...
async function pauseForApproval(trace) {
  ...
  if (shouldNotify("approval")) {
    ...
    await tgSend(
      trace.chatId,
      buildNotifyMessage({ type: "approval", title: "다음 단계 승인 대기", message: msg }),
      "HTML",
...
function buildSummaryReport(trace) {
  ...
  const stepsMark = trace.steps
    .map((s, idx) => (idx < trace.currentIndex ? `✔ ${labelStep(s)}` : idx === trace.currentIndex ? `⏳ ${labelStep(s)}` : `… ${labelStep(s)}`))
    .join(" → ");
