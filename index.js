...
// ✅ Responses API 우선 사용 + 자동 Fallback(chat.completions) + 스키마 검증(AJV)
// Node.js 18+, Express, axios, openai, ajv, ajv-formats
...
// 권장: Responses 모델(gpt-4.1-mini 등)과 Fallback 모델(gpt-4o-mini 등) 분리 설정
const OPENAI_MODEL_RESP = process.env.OPENAI_MODEL_RESP || "gpt-4.1-mini"; // Responses API용
const OPENAI_MODEL_FALLBACK = process.env.OPENAI_MODEL_FALLBACK || "gpt-4o-mini"; // chat.completions용
// 하위호환(이 값만 주는 환경도 동작하도록)
...
const MAX_REVISIONS = Number(process.env.MAX_REVISIONS || 3); // 단계별 승인 대기
...
// callback_query 응답 (버튼 로딩 해제용)
async function tgAnswerCallback(callbackQueryId, text = "", show_alert = false) {
  try {
...
function groupActive(limitPerBucket = 20) {
  ...
  for (const t of traces.values()) {
    ...
    else buckets.paused.push(snap); // unknown → paused로 분류
  }
...
// ========== OpenAI 공용 호출자 (Responses → Fallback 자동전환) ==========
async function callOpenAIJson({ system, user, schema, schemaName = "itplaylab_schema" }) {
  ...
  // 1) Responses API (권장) — 최신 openai SDK 필요
  try {
...
async function aiBrief(idea) {
  const { data, ok, latency_ms, provider, errors, raw_text, error } = await callOpenAIJson({
    system: "너는 콘텐츠 프로듀서다. 60초 쇼츠 중심으로 간결한 브리프를 JSON으로만 작성하라.",
    user: JSON.stringify(idea),
    schema: SCHEMA_BRIEF,
    schemaName: "content_brief",
  });
  return { ok, latency_ms, data: data || { raw_text }, provider, errors, error };
}
...
const revLabel = (trace) => (trace.revisionCount && trace.revisionCount > 0 ? `수정 #${trace.revisionCount}/${MAX_REVISIONS}` : "초안");
/* 구조
traces.set(traceId, {
  ...
  currentIndex: 0,
  approvalMode: true,
  history: [ { step, ok, latency_ms, error, startedAt, finishedAt, provider } ],
...
async function executeStep(trace, stepName) {
  ...
  try {
    ...
    if (!r.ok) {
      const reason = r.errors?.[0]?.message || r.error || "schema_validation_failed";
    ...
    if (shouldNotify("success")) {
      ...
      await tgSend(
        trace.chatId,
        buildNotifyMessage({ type: "success", title: stepTitle, message: msg })
      );
    }
    ...
    if (shouldNotify("error")) {
      ...
      await tgSend(
        trace.chatId,
        buildNotifyMessage({ type: "error", title: stepTitle, message: msg })
      );
    }
    throw e;
  }
...
function buildRevisionPrompts(stepName, trace, reason = "", checks = []) {
  ...
  const feedback = { reason, checks, title, profile };

  if (stepName === "brief") {
    return {
      system: "너는 콘텐츠 프로듀서다. 이전 브리프를 피드백에 맞게 최소 수정하여 더 나은 안을 JSON으로만 출력한다. 변경이유를 본문에 쓰지 말고 결과만 준수.",
      user: JSON.stringify({ mode: "revision", step: stepName, previous, feedback }),
      schema: SCHEMA_BRIEF,
      schemaName: "content_brief_revision",
    };
  }
  if (stepName === "script") {
    return {
      system: "너는 숏폼 스크립트라이터다. 이전 스크립트를 피드백에 맞게 수정한다. 총 60초, 샷당 3~6초 원칙 유지. JSON만 출력.",
      user: JSON.stringify({ mode: "revision", step: stepName, previous, brief: trace.lastOutput?.brief, feedback }),
      schema: SCHEMA_SCRIPT,
      schemaName: "content_script_revision",
    };
  }
  if (stepName === "assets") {
    return {
      system: "너는 유튜브 운영자다. 이전 메타/에셋을 피드백에 맞게 개선한다. 제목은 3안, 해시태그 5개. JSON만 출력.",
      user: JSON.stringify({ mode: "revision", step: stepName, previous, script: trace.lastOutput?.script, feedback }),
      schema: SCHEMA_ASSETS,
      schemaName: "content_assets_revision",
    };
  }
...
");
  ...
  const startedAt = nowISO();
  const prompts = buildRevisionPrompts(stepName, trace, reason, checks);
  const r = await callOpenAIJson(prompts);
  const latency_ms = r.latency_ms;
  if (!r.ok) throw new Error(r.errors?.[0]?.message || r.error || "revision_failed");
...
}_revision`, ok: true, latency_ms, provider: r.provider, startedAt, finishedAt: nowISO(), reason, by });

  await logToSheet({ type: `revision_${stepName}`, input_text: trace.title, output_text: r.data, project: PROJECT, category: "revision", note: `trace=${trace.id}`, latency_ms, trace_id: trace.id, step: stepName, ok: true, provider: r.provider, error: "" });

  // 알림 + 다시 승인 대기 상태로 전환 (currentIndex 그대로 유지)
  if (shouldNotify("success")) {
    const msg = [ fmtTitle(trace.title), fmtTrace(trace.id), `수정 단계: <b>${labelStep(stepName)}</b>`, `지연시간: <code>${latency_ms}ms</code>`, `엔진: <code>${r.provider}</code>` ].join("
");
    await tgSend(trace.chatId, buildNotifyMessage({ type: "success", title: `${labelStep(stepName)} 수정 완료`, message: msg }));
  }
  await pauseForApproval(trace);
...
");
    ...
    if (trace.currentIndex < trace.steps.length) {
      ...
      if (shouldNotify("success")) {
        ...
        await tgSend(
          trace.chatId,
          buildNotifyMessage({ type: "success", title: "출고 완료", message: msg })
        );
      }
    }
  }
...
function parseFreeText(text) {
  ...
  if (lower.includes("브리프")) {
    intent = "brief";
    steps = ["brief"];
  }
  if (lower.includes("스크립트")) {
    intent = "run_parts";
    steps = ["script"];
  }
...
app.post("/content/run", async (req, res) => {
  if (!requireOpenAI(res)) return;
  const started = Date.now();
  const { title, steps = ["brief", "script", "assets"], profile = "-", chatId = TELEGRAM_ADMIN_CHAT_ID } =
    req.body || {};
  ...
  const trace = {
    ...
    history: [],
    lastOutput: {},
    status: "initialized", revisionCount: 0,
  };
...
app.post("/reject", async (req, res) => {
  ...
  if (shouldNotify("approval")) {
    ...
    await tgSend(
      trace.chatId,
      buildNotifyMessage({ type: "error", title: "반려 처리됨", message: msg })
    );
  }
...
${fmtTrace(trace_id)}`);
      ...
      const summary = buildSummaryReport(trace);
      await tgSend(chatId, summary, "HTML");
      return res.json({ ok: true });
    }
